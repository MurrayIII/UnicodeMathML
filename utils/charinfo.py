...
mcdata = parse_file('https://www.unicode.org/Public/math/revision-15/MathClass-15.txt')
mathclasses = [mc for mc in mcdata if len(mc[0]) <= 5]
mathclassranges = [mc for mc in mcdata if len(mc[0]) > 5]
mathclassranges = [[mc[0].split('..'), mc[1]] for mc in mathclassranges if len(mc[0]) > 5]
...
var mathclasses = {
''')
[print("'{}':'{}',".format(*mathclass)) for mathclass in mathclasses]
print('''
}
var mathclassranges = [
''')
[print("[{},'{}'],".format(*mcr)) for mcr in mathclassranges]
print('''
]
function getCodepointData(cp) {
    ...
    if (!cpd)
        cpd = ['', 0, '']
    let mathclass = 'N'
    return {"name": cpd[0], "block": getBlock(cpd[1]), "category": cpd[2], "math-class": mathclass}
}
